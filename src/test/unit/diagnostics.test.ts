...
suite('Diagnostics', () => {
  ...
  suite('validateConfig', () => {
    ...
    test('errors when hostname set without protocol', () => {
      ...
      assert.ok(errors.some(e => e.message.includes('hostname') && e.message.includes('protocol')));
    });
    ...
    test('warns when hostname does not contain project name', () => {
      ...
      assert.ok(errors.some(e => e.message.includes('must contain project name') && e.severity === 'warning'));
    });
