...
import { getPorts, runDoctor, CliResult, CliError, PortsOutput, DoctorCheck, TunnelInfo } from '../cli';
...
export class OutportTreeProvider implements vscode.TreeDataProvider<OutportTreeItem> {
  ...
  private async fetchData(): Promise<void> {
    ...
    if (firstResult) {
      ...
      this.onDataLoaded?.({ ok: false, error: { kind: 'not-registered', message: 'Not registered' } });
    }
